...
loot spawn ~.5 ~.1 ~.5 loot ore_crops:items/diamond_seeds
particle minecraft:splash ~0.5 ~ ~0.5 0.1 0.1 0.1 0 50
playsound block.composter.fill_success block @a[distance=..50]
advancement grant @p only ore_crops:renewable_minerals
advancement grant @p only ore_crops:renewable_diamond
