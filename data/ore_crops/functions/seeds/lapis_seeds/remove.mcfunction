...
execute if score #success ore_crops.data matches 1 run loot spawn ~ ~ ~ loot ore_crops:items/lapis_seeds

execute store success score #success ore_crops.data run kill @e[type=item,nbt={Age:0s,Item:{id:"minecraft:wheat"}},distance=..1]
execute if score #success ore_crops.data matches 1 run loot spawn ~ ~ ~ loot ore_crops:drops/lapis
